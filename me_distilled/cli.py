...
def prompt_choice_back(question: str, options: list[str], default: int = 1, *, auto_yes: bool = False) -> int:
    """Like prompt_choice, but returns 0 when the user asks to go back."""
    if auto_yes:
        out(f"{question} -> {options[default - 1]}", "dim")
        return default
    out("")
    ...
    for idx, option in enumerate(options, 1):
        out(f"  {idx}. {option}")
    out("  b. 返回上一步")
    answer = input(f"请选择 [{default}]，或输入 b 返回: ").strip().lower()
    if answer in {"b", "back", "上一歩", "上一步", "返回"}:
        return 0
    if not answer:
        return default
    try:
...
def install_ollama() -> bool:
    ...
    for url in urls:
        ...
        proc = run_command(
            ["curl", "-L", "--fail", "--connect-timeout", "20", "--max-time", "600", url, "-o", str(archive)],
            check=False,
        )
        if proc.returncode != 0 or not archive.exists() or archive.stat().st_size < 1_000_000:
            out("这个 Ollama 下载源不可用，自动切换备用源。", "yellow")
            continue
        proc = run_command(["tar", "-xzf", str(archive), "-C", str(install_dir)], check=False)
        if proc.returncode != 0:
            out("Ollama 压缩包解压失败，自动切换备用源。", "yellow")
            continue
        candidates = [install_dir / "bin" / "ollama", install_dir / "ollama"]
        ollama_bin = next((item for item in candidates if item.exists()), None)
        if not ollama_bin:
            found = list(install_dir.rglob("ollama"))
            ollama_bin = found[0] if found else None
        if not ollama_bin:
            out("压缩包中没有找到 ollama 可执行文件，自动切换备用源。", "yellow")
            continue
        try:
            ...
            if os.geteuid() == 0:
                ...
                out(f"Ollama 已安装到 {target}", "green")
            else:
                out(f"Ollama 已下载到 {ollama_bin}。请把它加入 PATH。", "yellow")
        except Exception as exc:
            out(f"Ollama 已下载到 {ollama_bin}，但创建 PATH 链接失败: {exc}", "yellow")
...
def match_contacts(decrypted: Path, targets: list[str], include_groups: bool = False) -> list[dict[str, Any]]:
    ...
    for target in targets:
        ...
        if not candidates:
            ...
            continue
        def contact_rank(row: dict[str, str]) -> tuple[int, int, int, int, int, int, int, int]:
            name = row["name"].strip()
            aliases = row["aliases"]
            name_norm = name.lower()
            aliases_norm = aliases.lower()
            alias_parts = [part.strip().lower() for part in re.split(r"[,;/\s]+", aliases) if part.strip()]
            count = message_count(decrypted, row["wxid"])
            exact_wxid = int(target_norm == row["wxid"].lower())
            exact_name = int(target_norm == name_norm)
            exact_alias = int(target_norm in alias_parts)
            starts_name = int(name_norm.startswith(target_norm))
            contains_name = int(target_norm in name_norm)
            contains_alias = int(target_norm in aliases_norm)
            # Prefer exact and concise matches first: "妈" should rank above "妈妈真好".
            return (
                exact_wxid,
                exact_name,
                exact_alias,
        ...
        candidates.sort(key=contact_rank, reverse=True)
        chosen = candidates[0]
        result.append(
            {
                "target": target,
...
def doctor_lines() -> list[tuple[str, str, str]]:
    ...
    for name in ["git", "ollama", "node", "npm"]:
        if command_exists(name):
            ...
            note = "需要安装"
        checks.append((name, shutil.which(name) or "未找到", note))
...
def command_setup_deps(args: argparse.Namespace) -> None:
    ...
    if args.kind in {"train", "all"}:
        ...
        if module_exists("torchvision"):
            out("检测到 torchvision。当前训练链路不需要它，且部分版本会和 torch 冲突，正在移除。", "yellow")
            run_command([sys.executable, "-m", "pip", "uninstall", "-y", "torchvision"], check=False)
...
def wizard(args: argparse.Namespace) -> None:
    ...
    if prompt_yes("是否安装/检查 Ollama？", True, auto_yes=args.yes):
        command_setup_deps(argparse.Namespace(kind="system"))
    ...
    if not args.data_mode:
        mode_choice = prompt_choice_back(
            "训练数据类型",
            [
        ...
        if mode_choice == 0:
            return wizard(args)
    mode = args.data_mode or ["text-emoji-tag", "text-emoji", "sticker"][mode_choice - 1]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--kind", choices=["cli", "train", "system", "all"], default="cli")
